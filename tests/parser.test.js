...
const parser = require('../popup/parser.js');
...
describe('Test browser extension', () => {
	...
	beforeAll(async () => {
		const extensionEnvironment = await bootstrapExtension({
			pathToExtension: path.resolve('./popup',''), //The path to the uncompressed extension's folder. It shouldn't be a ZIP file.
			contentUrl: `https://rateyourmusic.com/`, // The URL of the content page that is being browsed
	...
	it("Should expect the title of the website", async () => {
        ...
        const bodyHTML = await contentPage.content();
        const bodyText = parser.parsePage(bodyHTML);
		expect(bodyText).toMatch('Rate Your Music');
	});
